...
export function AudioSetupModal({ exclusive, onExclusive, currentOffset, onApplyOffset, onClose }: {
  ...
  return <Modal open onClose={onClose} title="Audio setup & calibration" width="max-w-xl" footer={<Button variant="ghost" onClick={onClose}>Close</Button>}>
    ...
    <p className="mt-2 text-[11px] text-slate-400">Exclusive mode takes control of the default Windows output. Other apps may be silent until you switch back. Pitch-preserving playback uses shared audio.</p>
    ...
    <div className="my-5 rounded-xl border border-white/10 bg-white/[0.025] p-4">
      <h3 className="text-sm font-semibold text-slate-100">Find your timing offset</h3>
      <p className="mt-2 text-xs text-slate-400">Listen to four warm-up clicks, then tap Space with each of the next 16 clicks. Follow the sound. This estimates your tap alignment, including input and output delay; it is not a direct hardware latency measurement.</p>
      <p className="mt-2 text-xs text-slate-500">Current playtest offset: {currentOffset} ms. Recalibrate after changing headphones, output device or audio mode.</p>
