...
// .wrangler/tmp/bundle-gH2OOp/strip-cf-connecting-ip-header.js
...
// src/index.ts
var SESSION_COOKIE = "me_session";
var STATE_COOKIE = "me_oauth_state";
var SESSION_TTL_DAYS = 30;
var SUPABASE_TOKEN_TTL_SECONDS = 60 * 60;
var src_default = {
  async fetch(req, env) {
    ...
    try {
      ...
      return json({ error: message2 }, 500, env);
    }
  }
};
var OSZ_MIRRORS = [
...
var BEATMAP_LOOKUPS = [
  (id) => `https://catboy.best/api/v2/b/${id}`,
  (id) => `https://osu.direct/api/v2/b/${id}`
];
async function handleMirrorDownload(setId, env) {
...
__name(handleMirrorDownload, "handleMirrorDownload");
async function handleBeatmapLookup(beatmapId, env) {
...
__name(handleBeatmapLookup, "handleBeatmapLookup");
...
async function handleSession(req, env) {
  ...
  const uid = cookieToken && await verifySession(env, cookieToken) || headerToken && await verifySession(env, headerToken) || null;
  const clearCookies = cookieToken ? [cookie(SESSION_COOKIE, "", { maxAge: 0 })] : [];
  if (!uid)
    return json({ user: null }, 200, env, clearCookies);
...
__name(appAccessToken, "appAccessToken");
async function handleCover(req, env) {
  const cookieToken = getCookie(req, SESSION_COOKIE);
  const headerToken = bearerToken(req);
  const uid = cookieToken && await verifySession(env, cookieToken) || headerToken && await verifySession(env, headerToken) || null;
  if (!uid)
    return json({ cover_url: null }, 401, env);
  const user = await fetchUser(env, uid);
  if (!user)
    return json({ cover_url: null }, 404, env);
  const token = await appAccessToken(env);
  if (!token)
    return json({ cover_url: null }, 200, env);
  ...
  const cover = profile.cover?.custom_url ?? profile.cover?.url ?? profile.cover_url ?? null;
  return json({ cover_url: cover }, 200, env);
}
__name(handleCover, "handleCover");
...
async function upsertUser(env, fields) {
  const body = {
    ...fields,
  ...
  if (!res.ok)
    throw new Error(`user upsert failed: ${await res.text()}`);
...
async function fetchUser(env, id) {
  const res = await fetch(
...
async function mintSupabaseToken(env, user) {
  const secret = new TextEncoder().encode(env.SUPABASE_JWT_SECRET);
  return new SignJWT({
    role: "authenticated",
    is_admin: user.is_admin
  }).setProtectedHeader({ alg: "HS256", typ: "JWT" }).setSubject(user.id).setAudience("authenticated").setIssuedAt().setExpirationTime(`${SUPABASE_TOKEN_TTL_SECONDS}s`).sign(secret);
}
__name(mintSupabaseToken, "mintSupabaseToken");
async function signSession(env, uid) {
  const secret = new TextEncoder().encode(env.COOKIE_SECRET);
  return new SignJWT({ uid }).setProtectedHeader({ alg: "HS256", typ: "JWT" }).setIssuedAt().setExpirationTime(`${SESSION_TTL_DAYS}d`).sign(secret);
}
...
__name(cors, "cors");
function json(body, status, env, setCookies = []) {
  const headers = new Headers(cors(env));
...
__name(json, "json");
function redirect(location, env, setCookies) {
  const headers = new Headers(cors(env));
...
// .wrangler/tmp/bundle-gH2OOp/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gH2OOp/middleware-loader.entry.ts
